    ...
    <div className="hhBody">
      ...
      <aside className="hhPanel">
        <section className="hhCard hhFlowCard">
          ...
          <div className="hhStepBody">{tab==="decision"?<Decision strategy={strategy} setStrategy={setStrategy} onNext={()=>setTab("households")}/>:tab==="households"?<Households onNext={()=>setTab("sites")}/>:tab==="sites"?<Sites ranked={ranked} selected={selected} setSelected={setSelected} current={current} upgrades={upgrades} setUpgrades={setUpgrades} onNext={()=>setTab("future")}/>:tab==="future"?<Future ranked={ranked} current={current} rain={rain} setRain={setRain} growth={growth} setGrowth={setGrowth} roadFail={roadFail} setRoadFail={setRoadFail} onNext={()=>setTab("cohesion")}/>:<Consent current={current} briefHref={briefHref}/>}</div>
        </section>

        <section className="hhCard hhIndexCard">
          <div className="hhCardHead"><span>RELOCATION DECISION INDEX</span><button>How this is calculated</button></div>
          ...
          <div className="hhDrivers"><Driver label="Future hazard resilience" weight="44%" value={current.future}/><Driver label="Functional carrying capacity" weight="34%" value={Math.min(100,Math.round(functionalCapacity(current)/9.5))}/><Driver label="Community continuity" weight="22%" value={Math.round((current.cohesion+current.livelihood)/2)}/></div>
...
function Consent({current,briefHref}:{current:any;briefHref:any}){return <><span className="hhEyebrow">STEP 5 · CONSENT GATE</span><h2>Technical suitability cannot override people.</h2><p className="hhIntro">The plan remains a planning candidate until tenure and community consent are verified.</p><div className="hhMetricGrid"><Metric label="Neighbour clusters" value="88%"/><Metric label="School groups" value="93%"/><Metric label="Livelihood continuity" value={`${current.livelihood}%`}/><Metric label="Cohesion" value={`${current.cohesion}%`}/></div><div className="hhGate"><span>HARD GATE</span><b>Consent not yet verified</b><p>Recommendation may be technically viable, but execution is blocked.</p></div><Link href={briefHref} className="hhAnalyze link">Open auditable decision brief</Link></>}
