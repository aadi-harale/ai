...
export default async function Brief({searchParams}:{searchParams:Params}){
  ...
  const site=one(q.site,"B"),rain=one(q.rain,"20"),road=one(q.road,"0"),growth=one(q.growth,"10"),upgrade=one(q.upgrade,"0"),score=one(q.score,"—"),capacity=one(q.capacity,"—"),weakest=one(q.bottleneck,"Roads"),regret=one(q.regret,"—");
